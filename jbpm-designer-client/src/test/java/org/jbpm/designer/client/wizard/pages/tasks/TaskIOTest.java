...
public class TaskIOTest {
    ...
    @Test
    public void testRestrictDataTypesNullModel() throws Exception {
        when(dataBinder.getModel()).thenReturn(null);
        taskIO.restrictOutputDataTypes();
        verify(outputsTable).setAvailableDataTypes(dataTypes);
    }

    @Test
    public void testRestrictDataTypesNullOperation() throws Exception {
        ServiceTask model = mock(ServiceTask.class);
        when(model.getOperation()).thenReturn(null);
        when(dataBinder.getModel()).thenReturn(model);
        taskIO.restrictOutputDataTypes();
        verify(outputsTable).setAvailableDataTypes(dataTypes);
    }

    @Test
    public void testRestrictDataTypesNullResponse() throws Exception {
        ServiceTask model = mock(ServiceTask.class);
        Operation operation = mock(Operation.class);
        when(operation.getResponseScheme()).thenReturn(null);
        when(model.getOperation()).thenReturn(operation);
        when(dataBinder.getModel()).thenReturn(model);
        taskIO.restrictOutputDataTypes();
        verify(outputsTable).setAvailableDataTypes(dataTypes);
    }
    ...
    @Test
    public void testRestrictDataTypesNonExisting() throws Exception {
        ServiceTask model = mock(ServiceTask.class);
        Operation operation = mock(Operation.class);
        SwaggerSchema schema = mock(SwaggerSchema.class);
        when(schema.get$ref()).thenReturn("xyz");
        when(operation.getResponseScheme()).thenReturn(schema);
        when(model.getOperation()).thenReturn(operation);
        when(dataBinder.getModel()).thenReturn(model);
        taskIO.restrictOutputDataTypes();
        verify(outputsTable).setAvailableDataTypes(new ArrayList<String>());
    }

    @Test
    public void testRestrictDataTypes() throws Exception {
        ServiceTask model = mock(ServiceTask.class);
        Operation operation = mock(Operation.class);
        SwaggerSchema schema = mock(SwaggerSchema.class);
        when(schema.get$ref()).thenReturn("Integer");
        when(operation.getResponseScheme()).thenReturn(schema);
        when(model.getOperation()).thenReturn(operation);
        when(dataBinder.getModel()).thenReturn(model);
        taskIO.restrictOutputDataTypes();
        verify(outputsTable).setAvailableDataTypes(listCaptor.capture());
        Assert.assertEquals(1, listCaptor.getValue().size());
        Assert.assertEquals("Integer", listCaptor.getValue().get(0));
    }
}
